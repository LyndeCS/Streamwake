...
class ClientManager {
	constructor() {
		...
		this.sequelize = sequelize; // Use the shared Sequelize instance
		this.watchlist = [];
	...
	async loadWatchlist() {
		try {
			...
			this.watchlist = await this.watchlistModel.findAll();

			console.log(
				"Watchlist loaded and cached:",
				this.watchlist.map((item) => item.show_name)
			);
		} catch (error) {
